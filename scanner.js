...
const DEFAULT_WATCHLIST = ['R_50', '1HZ50V', 'R_75', '1HZ75V', '1HZ90V', 'R_100', '1HZ100V'];
...
function notifyPreview(setup) {
  ...
  const text =
    `📉 <b>${setup.directionLabel === 'BUY' ? 'BUY' : 'SELL'} SETUP DETECTED</b>\n` +
...
function notifySignal(setup) {
  ...
  const text =
    `${emoji} <b>${setup.directionLabel} SIGNAL CONFIRMED</b>\n` +
    `<b>${sd.label}</b> (${setup.symbol})\n` +
...
function notifyOutcome(setup, outcome, exitPrice) {
  ...
  const text =
    `${emoji} <b>${setup.directionLabel} SETUP ${outcome.toUpperCase()}</b>\n` +
    `<b>${sd.label}</b> (${setup.symbol})\n` +
