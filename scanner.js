...
function connect() {
  ...
  ws.on("open", () => {
    ...
    sendTelegram(
      wasReconnect
        ? "🔌 Reconnected to Deriv — connection is back up and running."
        : "🔌 Connected to Deriv — link is up and running. Loading instruments..."
    );
    if (SYMBOLS.length === 0) {
  ...
  ws.on("close", () => {
    ...
    if (!manualStop) {
      ...
      sendTelegram("⚠️ Connection to Deriv dropped — attempting to reconnect...");
      scheduleReconnect();
    } else {
...
function handleMessage(msg) {
  ...
  if (msg.msg_type === "active_symbols") {
    ...
    sendTelegram(`✅ Scanner connected — tracking ${SYMBOLS.length} instruments (${CFG.timeframe} timeframe).`);
    SYMBOLS.forEach((s) => enqueueSymbolRequests(s.symbol));
    runQueue();
    return;
  }
...
console.log(`[boot] Signal Scan starting — timeframe ${CFG.timeframe}, trend filter ${CFG.trendFilterEnabled}`);
sendTelegram(`🚀 Scanner service started (timeframe ${CFG.timeframe}). Connecting to Deriv...`);
connect();
