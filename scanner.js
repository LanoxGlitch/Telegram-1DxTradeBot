...
function fireSignal(symbol, direction, closedCandle) {
  const sd = DATA[symbol];
  const digits = closedCandle.close < 10 ? 5 : closedCandle.close < 1000 ? 3 : 2;
  const price = closedCandle.close.toFixed(digits);
  const emoji = direction === 'buy' ? '🟢' : '🔴';
  const text =
    `${emoji} <b>${direction.toUpperCase()} SIGNAL</b>\n` +
    `<b>${sd.label}</b> (${symbol})\n` +
...
}).listen(PORT, () => console.log(`[http] health check listening on :${PORT}`));
